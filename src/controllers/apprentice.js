...
const controllerApprentice = {
    ...
    certificationApprentice: async (req, res) => {
        const { id } = req.params
        const { certificationDoc, judymentPhoto } = req.body
        ...
        try {
            ...
            const apprentice = await Apprentice.findById(id);
            if (!apprentice) {
                return res.status(404).json({ message: 'No se encontró el aprendiz.' });
            }
            console.log(apprentice.HoursExecutedPS);


            if (apprentice.HoursExecutedPS !== 864) {
                return res.status(400).json({ message: 'El aprendiz no completado en totalidad sus horas de etapa productiva.' });
            }
            ...
            const binnacles = await Binnacles.find({ register: { $in: registerIds } }).sort({ createdAt: -1 });
            console.log(binnacles.length);
            const followups = await Followup.find({ register: { $in: registerIds } }).sort({ createdAt: -1 });
            console.log(followups.length);


            if (binnacles.length < 12 || followups.length < 3) {
                return res.status(400).json({ message: 'Para certificar el aprendiz, debe completar 12 bitácoras y 3 seguimientos.' });
            }

            const followupInstructor = await Register.findOne({
                _id: { $in: registerIds },
                "assignment": {
                  $elemMatch: {
                    followUpInstructor: {
              ...
              if (followupInstructor) {
                let hourFollowupPending = followupInstructor.hourFollowupPending || [];
              
                // Recorre cada assignment y filtra los followUpInstructor activos
                followupInstructor.assignment.forEach(assignment => {
                  const activeInstructors = assignment.followUpInstructor.filter(instructor => instructor.status === 1);
              
                  // Agrega los instructores activos al array hourFollowupPending
                  activeInstructors.forEach(instructor => {
                    hourFollowupPending.push({
                      idInstructor: instructor.idInstructor,
                      name: instructor.name,
                      hour: 2 // Asigna el número 2 como la hora
                    });
                  });
                });
              
                // Actualiza el registro con los datos acumulados de hourFollowupPending
                await Register.updateOne(
                  { _id: followupInstructor._id },
                  { $set: { hourFollowupPending: hourFollowupPending } }
                );
              
                console.log('hourFollowupPending actualizado:', hourFollowupPending);
              } else {
